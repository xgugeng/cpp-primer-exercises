...
They are all signed, with different minimum size for integer.
...
Typically, floats are represented in one word (32 bits), doubles in two words (64 bits). `float` usually does not have enough precision.

## Exercise 2.2

> To calculate a mortgage payment, what types would you use for the rate, principal, and payment? Explain why you selected each type.

Double for all, fore precision
...
It's legal, but the i in the for-loop is a local one covering the global i. i is 100 and sum is 45.
...
NO, the pointer points to a address, we need more info to detect it
...
`void*` is a special pointer type that can hold the address of any object, but the type of the object is unknown. While the type of `lp` is `long*`, it can only point to a long variable
...
(a) int* ip, i, &r = i;
ip is a pointer, i is an int, r is a reference to i

(b) int i, *ip = 0;
ip is an invalid pointer, i is an int

(c) int* ip, ip2;
ip is a pointer, ip2 in an int
...
p2 is a low-level const. p3 is both top-level and lowe-level, r2 is low-level const
...
Illegal, it might be `int null = 0, *p = nullptr`
...
j is int, k is const int&, p is a pointer to const int, j2 is const int, k2 is const int&
...
c is an int, b is int&. When the code finishes, all values is 4.
...
a:3, b:4, c:3, d:3, c is a int, d is int&.
